...
class modeloVendedor {
            ...
            if ($idProd) {
                ...
                if (!empty($resInv)) {
                    ...
                    if ($esSal) {
                        ...
                        // SOLUCIÓN RECUPERADA: SIEMPRE RESTAR KILOS
                        $pesoActual -= $kilos;
                        
                        if ($esPorPieza || $piezas > 0) {
                            if ($cajasActuales > 0) {
    ...
    public function obtenerNotasDelVendedor(int $idVendedor): array {
        // Obtenemos solo las notas en espera (GUARDADA) y ya no traemos las rechazadas
        $sql = "SELECT n.id_nota, n.folio, n.nombre_cliente, n.estado, n.fecha_creacion,
                ...
                WHERE n.id_vendedor = ? AND n.estado = 'GUARDADA'
                GROUP BY n.id_nota
    ...
    public function obtenerDatosNotaParaEditar(int $idNota): array {
        ...
        return [
            ...
            'observacion_especial' => $nota[0]['observacion_especial'] ?? '', 
