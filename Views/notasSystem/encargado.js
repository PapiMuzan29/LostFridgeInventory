...
window.switchTab = function (tabName, btnElement, subtitleText) {
  ...
  if (tabName === 'historial') {
      const inputBuscador = document.getElementById("buscarHistorial");
...
function cargarListaAutorizaciones() {
    // 1. Añadimos un timestamp para evadir caché
    const url = `/LostFridgeInventory/Controllers/encargadoController.php?action=listarAutorizaciones&_t=${Date.now()}`;
    ...
    fetch(url, { cache: 'no-store' })
        ...
        .then(data => {
            ...
            if (!data.success) {
                container.innerHTML = `<div class="empty-state"><p style="color:#e53e3e; font-weight:bold;">Error BD: ${data.message}</p></div>`;
                return;
            }
            ...
            container.innerHTML = data.notas.map(n => {
                const motivo = n.observacion_especial ? escapeHtml(n.observacion_especial) : 'Sin motivo registrado';
                const folio = escapeHtml(n.folio);
                const cliente = escapeHtml(n.nombre_cliente);

                return `
                <div class="card-inner" style="border: 1px solid var(--border-color); padding: 15px; margin-bottom: 12px; border-radius: 8px;">
                    ...
                    <div style="display:flex; justify-content: space-between; align-items: center; margin-bottom: 10px; border-bottom: 1px dashed var(--border-color); padding-bottom: 8px;">
                        <span class="badge badge-info"><i class="fa-solid fa-hashtag"></i> ${folio}</span>
                        <span style="font-weight: 600; color: var(--text-main); font-size: 0.9rem; text-align: right;">
                            <i class="fa-solid fa-user" style="color: var(--text-muted);"></i> ${cliente}
                        </span>
                    </div>
                    
                    <!-- Cuadro de Motivo adaptable a Modo Oscuro/Claro -->
                    <div style="background-color: rgba(49, 130, 206, 0.1); color: var(--text-main); padding: 10px 12px; border-left: 4px solid #3182ce; border-radius: 4px; font-size: 0.85rem; margin-bottom: 15px;">
                        <strong style="color: #3182ce;"><i class="fa-solid fa-lock"></i> Motivo Especial:</strong> 
                        <span style="display: block; margin-top: 4px; line-height: 1.4;">${motivo}</span>
                    </div>
                    
                    <!-- Botones con Íconos -->
                    <div style="display:flex; gap: 10px;">
                        <button type="button" class="btn-primary" style="flex:1; padding: 8px; font-size: 0.85rem; display: flex; justify-content: center; align-items: center; gap: 5px;" onclick="pedirPasswordParaNota(${n.id_nota})">
                            <i class="fa-solid fa-check"></i> Aprobar
                        </button>
                        <button type="button" class="btn-secondary-sm" style="flex:1; background-color: #e53e3e; color: white; border: none; padding: 8px; font-size: 0.85rem; display: flex; justify-content: center; align-items: center; gap: 5px;" onclick="denegarAutorizacion(${n.id_nota})">
        ...
        .catch(err => {
            ...
            document.getElementById('lista-autorizaciones-container').innerHTML = '<div class="empty-state"><p style="color:#e53e3e;">Error de red o Servidor.</p></div>';
        });
}
...
// ==========================================
// CARGAR HISTORIAL DE NOTAS APROBADAS Y RECHAZADAS (PAGINADO)
// ==========================================
function cargarHistorialAprobadas(textoBusqueda = "", pagina = 1) {
  ...
  const url = `/LostFridgeInventory/Controllers/encargadoController.php?action=listarHistorial&q=${encodeURIComponent(textoBusqueda)}&pagina=${pagina}`;

  if (textoBusqueda === "" && container.innerHTML.trim() === '') {
      container.innerHTML = `
        <div style="text-align:center; padding:20px;">
            <i class="fa-solid fa-spinner fa-spin fa-2x" style="color: var(--text-muted);"></i>
        </div>`;
    ...
    .then((data) => {
      ...
      if (data.success && data.notas && data.notas.length > 0) {
        data.notas.forEach((nota) => {
          // LÓGICA DE COLORES DEPENDIENDO DEL ESTADO
          const esAprobada = nota.estado === 'APROBADO';
          const colorBorde = esAprobada ? '#38a169' : '#e53e3e';
          const bgBadge = esAprobada ? '#c6f6d5' : '#ffebee';
          const colorBadge = esAprobada ? '#22543d' : '#c62828';
          const iconoBadge = esAprobada ? 'fa-check-double' : 'fa-xmark';
          const textoBadge = esAprobada ? 'Aprobada' : 'Rechazada';
          const iconoProducto = esAprobada ? 'fa-check' : 'fa-minus';

          // NUEVO: Mostrar quién la rechazó con todo y su rol
          const rolRechazo = nota.rol_quien_rechazo ? ` - ${escapeHtml(nota.rol_quien_rechazo)}` : '';
          const infoRechazo = nota.estado === 'RECHAZADA' && nota.nombre_quien_rechazo 
              ? `<div style="background: #ffebee; color: #c62828; padding: 8px 12px; border-radius: 6px; margin-top: 10px; font-size: 0.85rem; border: 1px solid #fc8181;">
                  <i class="fa-solid fa-ban"></i> <strong>Rechazada por:</strong> ${escapeHtml(nota.nombre_quien_rechazo)}${rolRechazo}
                 </div>` 
              : '';
          ...
          if (nota.productos && nota.productos.length > 0) {
            nota.productos.forEach((prod) => {
              const textoPiezas = prod.piezas > 0 ? ` / ${prod.piezas} pz` : "";
              listaProductosHTML += `<li><i class="fa-solid ${iconoProducto}" style="color:${colorBorde};"></i> ${escapeHtml(prod.nombreProducto)} (${prod.kilos} kg${textoPiezas})</li>`;
            });
          }

          nuevoHTML += `
            <div class="note-card card-inner fade-in-up" style="border-left: 4px solid ${colorBorde}; opacity: 0.9;">
                <div class="note-header">
                    <span class="badge badge-info"><i class="fa-solid fa-hashtag"></i> ${escapeHtml(nota.folio)}</span>
                    <span class="badge" style="background:${bgBadge}; color:${colorBadge};"><i class="fa-solid ${iconoBadge}"></i> ${textoBadge}</span>
                </div>
                <div class="note-body">
                    <p class="note-cliente"><i class="fa-solid fa-user"></i> <strong>${escapeHtml(nota.nombre_cliente)}</strong></p>
                    <p class="note-date"><i class="fa-regular fa-calendar-check"></i> ${escapeHtml(nota.fecha_creacion)}</p>
        ...
        if (paginacionContainer) {
            renderizarPaginacionUniversal(
                ...
                (nuevaPagina) => cargarHistorialAprobadas(textoBusqueda, nuevaPagina)
            );
        }
        ...
        nuevoHTML = `
          <div class="empty-state fade-in-up">
              ...
              <p>No se encontraron notas en el historial.</p>
          </div>`;
        if (paginacionContainer) paginacionContainer.innerHTML = "";
      }
      container.innerHTML = nuevoHTML;
...
function filtrarHistorial() {
  ...
  timeoutBusquedaHistorial = setTimeout(() => {
    cargarHistorialAprobadas(valor, 1); // Siempre lo regresa a la página 1 al buscar
  }, 400);
}